...
echo "Generate route configs with car probability from ${car_from} to ${car_to} with step ${car_step}"
...
for parking_probability in $(seq ${parking_from} ${parking_step} ${parking_to}); do
	...
	#	echo ${sumo_port}
		...
		cat ${omnet_template} | sed "s/{SUMO_CONFIG}/${sumo_file}/;s/{SUMO_PORT}/${sumo_port}/;s/{PARKING_PROBABILITY}/${parking_probability}/" > omnetpp-car-${car_probability}-parking-${parking_probability}.ini
	done;
